...
import re
#IT IS THE SAME AS MerkurWin

class AdmiralyesSpider(scrapy.Spider, TemplateSpider):
    name = 'AdmiralYes'
    allowed_domains = ['www.admiralyes.it', 'sport-isibet.admiralyes.it']
    ...
    def start_requests(self):   #sends a request to get the tree (it contains all the info to find urls)
        yield scrapy.Request(url='https://sport-isibet.admiralyes.it/backend/api/SportInfos/TournamentsTree', 
                            callback = self.parse)
    ...
    def parse(self, response):
        ...
        for country in jsonResponseSports.get("sports").get('1').get("categories").values():    #football is code 1, categories are countries (Italy, Spain...)
            self.now = datetime.now()   #get current time to compare it with event date and time (past and liove events must be discarded)
            for tournament in country.get('tournaments').values():
                ...
                if sum(tournament.get('eventsCounter').values()) > self.MINIMUM_NUMBER_OF_BETS: #finds the number of events in the tournament
                    yield scrapy.Request(
                        url = f'https://sport-isibet.admiralyes.it/backend/api/SportInfos/TournamentEvents?sportCode={sportCode}&tournamentCode={tournamentCode}&betCode={betCode}&betSection={betSection}&tag=',
    ...
    def parseTournament(self, response):
        ...
        for eventKey in jsonResponseMatchday.get("body").keys():    #each event has a code in the format scheduleCode.eventCode
            if jsonResponseMatchday.get("body").get(eventKey).get("liveStatus") == None:
                scheduleCode , eventCode = eventKey.split('.', 1)  #splits the codes (maxsplit = 1 to make sure only two string are generated)
                yield scrapy.Request(
                    url = f'https://sport-isibet.admiralyes.it/backend/api/SportInfos/EventDetails?scheduleCode={scheduleCode}&eventCode={eventCode}',
    ...
    def parseEventBets(self, response):
        ...
        if dateUTC < self.now:
            ...
            return
        try:
            eventName = self.findEventName(eventBody.get('name'), self.splitterAdmiralYes)
            ...
            for betCode in eventBody.get('bets').keys():    #betCode is the code of betType (example: 1X2 has code 3)
                ...
                #checks if th ebet is excluded (type 'MARCATORE' excluded to lighten the software)
        ...
        except (AttributeError,ValueError): #AttributeError: event without betRadarID, ValueError: bets not of a match (may be antepost)
            pass
    ...
    def isExcludedType(self, betType):
        for excludedType in self.EXCLUDED_BET_TYPES:
            if excludedType in betType: #for example if the word MARCATORE appears in betType (e.g. MARCATORE SI/NO) the bet is excluded
                return True
        return False
